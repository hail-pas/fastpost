...
class RecursionLimitPydanticMeta(PydanticMeta):
    max_recursion: int = 1
...
def pydantic_model_creator(
    ...
    *,
    ...
    include: Tuple[str, ...] = (),
    computed: Tuple[str, ...] = (),
    allow_cycles: Optional[bool] = False,
    sort_alphabetically: Optional[bool] = None,
    _stack: tuple = (),
    ...
    def get_name() -> str:
        ...
        nonlocal postfix
        is_default = (
            exclude == () and include == () and computed == () and sort_alphabetically is None and allow_cycles is None
        )
    ...
    def get_param(attr: str) -> Any:
        if meta_override:
            return getattr(meta_override, attr, getattr(meta, attr, getattr(RecursionLimitPydanticMeta, attr)))
        return getattr(meta, attr, getattr(RecursionLimitPydanticMeta, attr))
    ...
    pconfig: Type[pydantic.main.BaseConfig] = type(
        ...
        {"title": name or cls.__name__, "extra": pydantic.main.Extra.allow, "fields": {}},
    )
    ...
    if not exclude_readonly:
        ...
        field_map.update(
            {k: {"field_type": callable, "function": getattr(cls, k), "description": None} for k in computed}
        )
